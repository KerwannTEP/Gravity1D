...
function MinHeap()
    return MinHeap(PREC_FLOAT[], Int64[], Int64[])
end
...
# Insert a new value
function push!(heap::MinHeap, val::PREC_FLOAT, index::Int64)
...
function replace!(heap::MinHeap, i::Int, new_val::PREC_FLOAT)
    old_val = heap.data[i]
    heap.data[i] = new_val
    if new_val < old_val
        bubble_up!(heap, i)
